 ...
 *
 */
contract AaveV3LeverageStrategyExtension is BaseExtension {
    ...
    struct ActionInfo {
        uint256 collateralBalance;                      // Balance of underlying held in AaveV3 in base units (e.g. USDC 10e6)
        uint256 borrowBalance;                          // Balance of underlying borrowed from AaveV3 in base units
    ...
    struct ContractSettings {
        ...
        ILeverageModule leverageModule;                 // Instance of AaveV3 leverage module
        IProtocolDataProvider aaveProtocolDataProvider; // Instance of AaveV3 protocol data provider
    ...
    /**
     * OPERATOR ONLY: Engage to target leverage ratio for the first time. SetToken will borrow debt position from AaveV3 and trade for collateral asset. If target
     ...
     * collateral position and trade for debt position to repay AaveV3. If the chunk rebalance size is less than the total notional size, then this function will
     * delever and repay entire borrow balance on AaveV3. If chunk rebalance size is above max borrow or max trade size, then operator must
    ...
    /**
     * Calculate notional rebalance quantity, whether to chunk rebalance based on max trade size and max borrow and invoke lever on AaveV3LeverageModule
     *
     */
     function _lever(
    ...
    /**
     * Calculate delever units Invoke delever on AaveV3LeverageModule.
     */
    function _delever(
        LeverageInfo memory _leverageInfo,
        uint256 _chunkRebalanceNotional
    ...
    /**
     * Invoke deleverToZeroBorrowBalance on AaveV3LeverageModule.
     */
     ...
     * The collateral value in USD * AaveV3 collateral factor * (1 - unutilized leverage %)
     *
     * Net repay limit for delevering is calculated as:
     * The collateral value in USD * AaveV3 liquiditon threshold * (1 - unutilized leverage %)
     *
    ...
    function _calculateMaxBorrowCollateral(ActionInfo memory _actionInfo, bool _isLever) internal view returns(uint256) {
        ...
        // Normalize LTV and liquidation threshold to precise units. LTV is measured in 4 decimals in AaveV3 which is why we must multiply by 1e14
