 ...
 * All swaps are done using the best price found on 0x.
 *
 */
contract ExchangeIssuanceZeroEx is ReentrancyGuard {
    ...
    address public immutable zeroExFactory;
    ...
    constructor(
        address _weth,
        address _zeroExFactory,
        IController _setController,
        IBasicIssuanceModule _basicIssuanceModule
    )
        public
    {
        zeroExFactory = _zeroExFactory;
    ...
    */
    ...
    /**
     * Swaps a given amount of an ERC20 token for WETH for the best price on 0x.
     *
    ...
    function _swapTokensForExactTokens(Exchange _exchange, address _tokenIn, address _tokenOut, uint256 _amountOut) internal returns (uint256) {
        ...
        path[0] = _tokenIn;
        path[1] = _tokenOut;
        return zeroExRouter.swapTokensForExactTokens(_amountOut, PreciseUnitMath.maxUint256(), path, address(this), block.timestamp)[0];
    }
    ...
    function _getMinTokenForExactToken(uint256 _amountOut, address _tokenA, address _tokenB) internal view returns (uint256, Exchange, address) {
        ...
        address zeroExPair = _getPair(zeroExFactory, _tokenA, _tokenB);
        ...
        // Fails if the values are maxIn
        require(tokenIn < maxIn, "ExchangeIssuance: ILLIQUID_SET_COMPONENT");
        return (tokenIn, Exchange.ZeroEx, zeroExPair);
    }
    ...
    function _getMaxTokenForExactToken(uint256 _amountIn, address _tokenA, address _tokenB) internal view returns (uint256, Exchange, address) {
        ...
        // TODO: write 0x version
        uint256 tokenOut = 0;

        address zeroExPair = _getPair(zeroExFactory, _tokenA, _tokenB);

        // Fails if token out is <= 0
        require(tokenOut > 0);
        return (tokenOut, Exchange.ZeroX, zeroXPair);
    }
    ...
    function _getPair(address _factory, address _tokenA, address _tokenB) internal view returns (address) {
        return IZeroExFactory(_factory).getPair(_tokenA, _tokenB);
    }
}
