...
if (process.env.INTEGRATIONTEST) {
  describe.only("PrtStakingPool HyETH - Integration Test", async () => {
    ...
    before(async () => {
      [owner, bob, alice, carol] = await getAccounts();
    ...
    context(
      "When the PRT, PRT Staking Pool, and Prt Fee Split Extension are deployed and setup",
      () => {
        ...
        before(async () => {
          ...
          await prtFeeSplitExtension
            .connect(operator)
            .updateFeeRecipient(prtFeeSplitExtension.address);
          await prtFeeSplitExtension
            .connect(methodologist)
            .updateFeeRecipient(prtFeeSplitExtension.address);

          console.log("deploy stakng pool", [
            prtStakingPoolName,
        ...
        it("should set the PrtFeeSplitExtension state correctly", async () => {
          ...
          expect(await prtFeeSplitExtension.streamingFeeModule()).to.eq(
            addresses.setFork.streamingFeeModule,
          );
          expect(await prtFeeSplitExtension.issuanceModule()).to.eq(
            addresses.setFork.debtIssuanceModuleV2_1,
          );
          expect(await prtFeeSplitExtension.operatorFeeSplit()).to.eq(prtFeeSplit);
          expect(await prtFeeSplitExtension.operatorFeeRecipient()).to.eq(
            addresses.setFork.feeRecipient,
          );
        ...
        context("When the PRTs are distributed and staked", () => {
          ...
          context("When the first snapshot is taken", () => {
            ...
            it("should allow bob and alice to claim proportional rewards", async () => {
              ...
              const expectedAliceRewards = accruedFees
                .mul(alicePrtAmount)
                .div(totalStakedPrtAmount);

              const bobPendingRewardsBefore = await prtStakingPool.getPendingRewards(bob.address);
              const alicePendingRewardsBefore = await prtStakingPool.getPendingRewards(
                alice.address,
              );
              ...
              const alicePendingRewardsAfter = await prtStakingPool.getPendingRewards(
                alice.address,
              );
            ...
            context("When the second snapshot is taken", () => {
              ...
              it("should allow bob, alice, and carol to claim proportional rewards", async () => {
                ...
                const expectedBobRewards = accruedFeesSnapshotTwo
                  .mul(bobPrtAmount)
                  .div(totalStakedPrtAmount);
                const expectedAliceRewards = accruedFeesSnapshotTwo
                  .mul(alicePrtAmount)
                  .div(totalStakedPrtAmount);

                const expectedCarolSnapshotOneRewards = accruedFeesSnapshotOne
                  .mul(carolPrtAmount)
                  .div(totalStakedPrtAmount);
                const expectedCarolSnapshotTwoRewards = accruedFeesSnapshotTwo
                  .mul(carolPrtAmount)
                  .div(totalStakedPrtAmount);
                const expectedCarolRewards = expectedCarolSnapshotOneRewards.add(
                ...
                const carolPendingRewardsBefore = await prtStakingPool.getPendingRewards(
                  carol.address,
                );
                ...
                const carolPendingRewardsAfter = await prtStakingPool.getPendingRewards(
                  carol.address,
                );
                ...
                expect(bobSetTokenBalanceAfter).to.eq(
                  expectedBobRewards.add(bobSetTokenBalanceBefore),
                );
                expect(aliceSetTokenBalanceAfter).to.eq(
                  expectedAliceRewards.add(aliceSetTokenBalanceBefore),
                );
              ...
              context("When the third snapshot is taken", () => {
                ...
                it("should allow bob, alice, and carol to claim proportional rewards", async () => {
                  ...
                  const expectedAliceRewards = accruedFeesSnapshotThree
                    .mul(alicePrtAmount)
                    .div(totalStakedPrtAmount);
                  const expectedCarolRewards = accruedFeesSnapshotThree
                    .mul(carolPrtAmount)
                    .div(totalStakedPrtAmount);

                  const bobPendingRewardsBefore = await prtStakingPool.getPendingRewards(
                    bob.address,
                  );
                  const alicePendingRewardsBefore = await prtStakingPool.getPendingRewards(
                    alice.address,
                  ...
                  expect(carolSetTokenBalanceAfter).to.eq(
                    expectedCarolRewards.add(carolSetTokenBalanceBefore),
                  );
                });
              });
            });
