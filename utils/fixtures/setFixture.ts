...
export class SetFixture {
  ...
  public async initialize(): Promise<void> {
    ...
    this.integrationRegistry = await this._deployer.setV2.deployIntegrationRegistry(
      this.controller.address,
    );
    this.factory = await this._deployer.setV2.deploySetTokenCreator(this.controller.address);

    this.issuanceModule = await this._deployer.setV2.deployBasicIssuanceModule(
      this.controller.address,
    );
    this.streamingFeeModule = await this._deployer.setV2.deployStreamingFeeModule(
      this.controller.address,
    );
    this.debtIssuanceModule = await this._deployer.setV2.deployDebtIssuanceModule(
      this.controller.address,
    );
    this.governanceModule = await this._deployer.setV2.deployGovernanceModule(
      this.controller.address,
    );
  ...
  public async createSetToken(
    ...
    const retrievedSetAddress = await new ProtocolUtils(this._provider).getCreatedSetTokenAddress(
      txHash.hash,
    );
  ...
  public async approveAndIssueSetToken(
    ...
    useDebtIssuance: boolean = false,
  ): Promise<any> {
    const positions = await setToken.getPositions();
    const issuanceModule = useDebtIssuance ? this.debtIssuanceModule : this.issuanceModule;
    for (let i = 0; i < positions.length; i++) {
      const { component } = positions[i];
      const componentInstance = await this._deployer.setV2.getTokenMock(component);
      await componentInstance.approve(issuanceModule.address, MAX_UINT_256);
    }

    await issuanceModule.issue(setToken.address, issueQuantity, to);
  }
}
